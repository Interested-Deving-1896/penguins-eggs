#!/usr/bin/env bash
# scripts/sync-all-features.sh — delegation shim
#
# The canonical implementation lives in:
#   Interested-Deving-1896/fork-sync-all  scripts/sync-all-features.sh
#
...
# repo) work without modification. In CI the sync-all-features.yml workflow
...
#   bash scripts/sync-all-features.sh
#
...
if [[ -n "$FORK_SYNC_ALL" && -f "${FORK_SYNC_ALL}/scripts/sync-all-features.sh" ]]; then
  echo "[shim] delegating to ${FORK_SYNC_ALL}/scripts/sync-all-features.sh"
  ...
  exec bash "${FORK_SYNC_ALL}/scripts/sync-all-features.sh" "$@"
fi
...
echo "[shim] FORK_SYNC_ALL not set — triggering sync-all-features workflow in fork-sync-all"
...
curl -fsSL \
  ...
  "https://api.github.com/repos/${GITHUB_OWNER}/fork-sync-all/actions/workflows/sync-all-features.yml/dispatches" \
  -d "{
    \"ref\": \"main\",
    \"inputs\": {
